...
#if 0
    gpio_config(UART_CTS, INPUT, PULL_HIGH);			//mcu uart idle 0:en 1:dis
    gpio_config(MODE_CTRL, INPUT, PULL_HIGH);			//control mode    0:cmd mode 1:trans mode
    gpio_config(RX_WAKEUP, INPUT, PULL_HIGH);			//wakeup of uart 0:active 1:sleep
    gpio_config(BT_EN, INPUT, PULL_HIGH);				//enable of ble 0:on 1:off
    gpio_config(UART_RTS, OUTPUT, PULL_NONE);			//ble uart idle 0:en 1:dis
	set_rts_busy(1);
	
    gpio_config(WORK_STAT, OUTPUT, PULL_NONE);//ble init state 0:no 1:yes
	set_module_ready(0);
    gpio_config(CONN_STA1, OUTPUT, PULL_NONE);//ble state 00:disconn 01:pairing 10:need key pair 11:connected
	gpio_config(CONN_STA0, OUTPUT, PULL_NONE);
