                         ...
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    #if (RW_DEBUG_NVDS && NVDS_READ_WRITE)
 ...
 */
static int
hci_dbg_hw_reg_rd_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_HW_REG_RD_CMD_OPCODE, hci_dbg_hw_reg_rd_cmd_cmp_evt);

    event->reg_value = co_htobl(REG_BLE_RD(REG_BLECORE_BASE_ADDR + co_btohs(param->reg_addr)));
 ...
 */
static int
hci_dbg_hw_reg_wr_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    // allocate the status event message
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_HW_REG_WR_CMD_OPCODE, hci_dbg_hw_reg_wr_cmd_cmp_evt);

    #if (BLE_EMB_PRESENT)
    REG_BLE_WR(REG_BLECORE_BASE_ADDR + co_btohs(param->reg_addr), co_btohl(param->reg_value));
    #endif //BLE_EMB_PRESENT
 ...
 */
static int
hci_dbg_set_bd_addr_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    // Sends the CC event
    event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_SET_BD_ADDR_CMD_OPCODE, hci_basic_cmd_cmp_evt);
    event->status = CO_ERROR_NO_ERROR;
    hci_send_2_host(event);
 ...
 */
static int
hci_dbg_set_crc_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    // allocate the status event message
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_SET_CRC_CMD_OPCODE, hci_basic_cmd_cmp_evt);
    event->status = CO_ERROR_NO_ERROR;
    // sends the message
    hci_send_2_host(event);
 ...
 */
static int
hci_dbg_llcp_discard_cmd_handler(ke_msg_id_t const msgid,
                                ...
                                ke_task_id_t const src_id)
{
    ...
    // allocate the status event message
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_LLCP_DISCARD_CMD_OPCODE, hci_basic_cmd_cmp_evt);
    event->status = CO_ERROR_NO_ERROR;
    // sends the message
    hci_send_2_host(event);
 ...
 */
static int
hci_dbg_wr_dle_dft_value_cmd_handler(ke_msg_id_t const msgid, struct hci_dbg_wr_dle_dft_value_cmd const *param,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{

    // structure type for the complete command event
    struct hci_basic_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_WR_DLE_DFT_VALUE_CMD_OPCODE, hci_basic_cmd_cmp_evt);
 ...
 */
static int
hci_dbg_reset_rx_cnt_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    // allocate the status event message
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_RESET_RX_CNT_CMD_OPCODE, hci_basic_cmd_cmp_evt);
 ...
 */
static int
hci_dbg_reset_tx_cnt_cmd_handler(ke_msg_id_t const msgid,
                         ...
                         ke_task_id_t const src_id)
{
    ...
    // allocate the status event message
    event =  KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_RESET_TX_CNT_CMD_OPCODE, hci_basic_cmd_cmp_evt);
 ...
 */
static int
hci_dbg_set_tx_pw_cmd_handler (ke_msg_id_t const msgid,
                            struct hci_dbg_set_tx_pw_cmd const *param,
                            ke_task_id_t const dest_id,
                            ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_basic_cmd_cmp_evt *event= KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , src_id, HCI_DBG_SET_TX_PW_CMD_OPCODE, hci_basic_cmd_cmp_evt);
 ...
 */
static int
hci_vs_audio_configure_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_configure_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_basic_conhdl_cmd_cmp_evt *event =
            KE_MSG_ALLOC(HCI_CMD_CMP_EVENT , param->conhdl, msgid, hci_basic_conhdl_cmd_cmp_evt);
 ...
 */
static int
hci_vs_audio_set_pointer_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_set_pointer_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_basic_conhdl_cmd_cmp_evt *event =
 ...
 */
static int
hci_vs_audio_get_buffer_range_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_get_buffer_range_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_vs_audio_get_buffer_range_cmd_cmp_evt *event = 
            KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, msgid, hci_vs_audio_get_buffer_range_cmd_cmp_evt);
 ...
 */
static int
hci_vs_audio_set_mode_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_set_mode_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_basic_conhdl_cmd_cmp_evt *event =
 ...
 */
static int
hci_vs_audio_reset_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_reset_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_basic_conhdl_cmd_cmp_evt *event =
 ...
 */
static int
hci_vs_audio_allocate_cmd_handler(ke_msg_id_t const msgid, struct hci_vs_audio_allocate_cmd const *param,
                         ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_vs_audio_allocate_cmd_cmp_evt *event =
            KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, param->conhdl, msgid, hci_vs_audio_allocate_cmd_cmp_evt);
...
static const struct ke_msg_handler dbg_hci_command_handler_tab[] =
{
        ...
        #if (BLE_EMB_PRESENT)
        {HCI_DBG_HW_REG_RD_CMD_OPCODE,      (ke_msg_func_t)hci_dbg_hw_reg_rd_cmd_handler},
        {HCI_DBG_HW_REG_WR_CMD_OPCODE,      (ke_msg_func_t)hci_dbg_hw_reg_wr_cmd_handler},
        {HCI_DBG_SET_BD_ADDR_CMD_OPCODE,    (ke_msg_func_t)hci_dbg_set_bd_addr_cmd_handler},
        {HCI_DBG_SET_CRC_CMD_OPCODE,        (ke_msg_func_t)hci_dbg_set_crc_cmd_handler},
        #if (BLE_PERIPHERAL || BLE_CENTRAL)
        {HCI_DBG_LLCP_DISCARD_CMD_OPCODE,   (ke_msg_func_t)hci_dbg_llcp_discard_cmd_handler},
        #endif // #if (BLE_PERIPHERAL || BLE_CENTRAL)
        {HCI_DBG_RESET_RX_CNT_CMD_OPCODE,   (ke_msg_func_t)hci_dbg_reset_rx_cnt_cmd_handler},
        {HCI_DBG_RESET_TX_CNT_CMD_OPCODE,   (ke_msg_func_t)hci_dbg_reset_tx_cnt_cmd_handler},
        #if (BLE_OBSERVER || BLE_CENTRAL || BLE_PERIPHERAL)
        {HCI_DBG_SET_TX_PW_CMD_OPCODE,      (ke_msg_func_t)hci_dbg_set_tx_pw_cmd_handler},
        #endif //(BLE_OBSERVER || BLE_CENTRAL || BLE_PERIPHERAL)
        #endif //BLE_EMB_PRESENT
        #if (RW_WLAN_COEX)
        ...
        #if (BLE_EMB_PRESENT)
        #if (BLE_PERIPHERAL || BLE_CENTRAL)
        {HCI_DBG_WR_DLE_DFT_VALUE_CMD_OPCODE, (ke_msg_func_t)hci_dbg_wr_dle_dft_value_cmd_handler},
        #if(BLE_AUDIO)
        {HCI_VS_AUDIO_ALLOCATE_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_allocate_cmd_handler},
        {HCI_VS_AUDIO_CONFIGURE_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_configure_cmd_handler},
        {HCI_VS_AUDIO_SET_MODE_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_set_mode_cmd_handler},
        {HCI_VS_AUDIO_RESET_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_reset_cmd_handler},
        {HCI_VS_AUDIO_SET_POINTER_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_set_pointer_cmd_handler},
        {HCI_VS_AUDIO_GET_BUFFER_RANGE_CMD_OPCODE, (ke_msg_func_t)hci_vs_audio_get_buffer_range_cmd_handler},
        #endif // #if (BLE_AUDIO)
        #endif // #if (BLE_PERIPHERAL || BLE_CENTRAL)
        #endif
};
