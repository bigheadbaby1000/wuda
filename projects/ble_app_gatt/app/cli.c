...
*/
uint8_t lesend_send1=0;
uint8_t lesend_send2=0;
uint16_t lesend_len1=0;
static int handle_input(char *inbuf,int len)
{
    ...
    do {
        ...
        switch (inbuf[i]) {
            case '\0':
                if((lesend_send1 == 1)&&(lesend_send2 != 1) && (argc >= 3))
                {
                    lesend_send2 = 1;
                    lesend_len1 = atoi(argv[2]);
                    
                    stat.inArg = 1;
                    argc++;
                    argv[argc - 1] = &inbuf[i];
                    UART_CLI_PRINTF("in \\0\r\n");
                    break;
                }
                if (stat.inQuote) {
                    return 2;
            ...
            //case '"':
            case ',':
                UART_CLI_PRINTF("%x-", inbuf[i-1]);
                if((lesend_send1 == 1)&&(lesend_send2 != 1) && (argc >= 2)&&(inbuf[i-1] == '\0'))
                {
                    lesend_send2 = 1;
                    lesend_len1 = atoi(argv[2]);
...
//                if(strncmp(argv[0],"AT+LESEND",9)==0)lesend_send1 = 1;//!strncmp(cli->commands[i]->name, name, len)
                if (i > 0 && inbuf[i - 1] == '\\' && stat.inArg) {
            ...
            default:
                if (!stat.inArg) {
                    stat.inArg = 1;
                    argc++;
                    UART_CLI_PRINTF("argc++ %d\r\n",argc);
                    if((strncmp(argv[0],"AT+LESEND",9)==0) && (argc >= 3))lesend_send1 = 1;//
                    if((argc == 4) && (lesend_send1 == 1))
                    {
                        lesend_send2 = 1;
                        lesend_len1 = atoi(argv[2]) - 1;
                        UART_CLI_PRINTF("argc 4 %02x %d ", inbuf[i], lesend_len1);
                    }
                    argv[argc - 1] = &inbuf[i];
                }
                break;
...
//	set_rts_busy(1);
	...
	if(!send_data_enable_flag)		//if function is send msg, send_data_enable_flag =1
	set_rts_busy(0);
 ...
 */
static int get_input(char *inbuf, unsigned int *bp, volatile uint16_t *len)//
{
    //char c;
    //int  esc = 0, key1 = -1, key2 = -1;
    uint8_t flag = 0, char_num=0;
    ...
    while (cli_getchar(&inbuf[*bp]) == 1) {
        ...
        //USER_PRINTF("%x %02x\r\n",bp[0], inbuf[*bp]);
//                Delay_ms(1);
                if((len[0] > 12) && (strncmp(&inbuf[0],"AT+LESEND",9)==0))
                {       
                    //Delay_ms(1);      
                    //USER_PRINTF("%02x %d\r\n",inbuf[*bp], flag);
                    if((',' == inbuf[*bp]) && (char_num == 0))
                    {
                        char_num = bp[0];
                    }
                    temp_len = atoi(&inbuf[12]);
                    //USER_PRINTF("cli:%d %d %x %x\r\n",len[0],temp_len,char_num,bp[0]);
                    if(len[0] <= (temp_len+char_num))flag = 1;
                    else flag = 0;
                }
                ...
                if (!flag)
                {
                    ...
                    {                        
                            ...
                            *bp = 0;
                            uart_ringbuf_clean();
                        //USER_PRINTF("uart_ringbuf_clean\r\n");
                            return 1;
                    }
                }
			#else
...
//            fflush(stdout);
        ...
        (*len)++;
        if (*bp >= INBUF_SIZE) {
            UART_CLI_PRINTF("Error: input buffer overflow = %d\r\n", *bp);
...
//    aos_cli_printf("cli_main\r\n");
    ...
    {		
        ...
        if (get_input(cli->inbuf, &cli->bp, &len)) {
            msg = cli->inbuf;
#if 0
...
//    aos_cli_printf("cli:0x%x \r\n",cli);
    ...
    if (cli == NULL) {
			 UART_CLI_PRINTF("Error0: Failed to create cli env: %d\r\n",
                       -ENOMEM);
			  UART_CLI_PRINTF("Error1: Failed to create cli env: %d\r\n",
                       -ENOMEM);
        return -ENOMEM;
    }else
    {
...
init_general_err:
	...
	UART_CLI_PRINTF("Error: return: %d\r\n",
                       ret);
...
/*
read 1byte data from ringbuf until no data can be read and return 0.
*/
int cli_getchar(char *inbuf)
{
	...
	//*inbuf = Read_Uart_Buf();
    uint8_t len;
    len = read_uart_ringbuf_data((uint8_t*)inbuf,1);	//Return: uint8_t readed cnt
	if(len != 0)
	{
		return 1;
	}else {
